...
public record Address(
        ...
        @Pattern(regexp = "^[A-Z0-9\\-\\s]+$", message = "Postal code must contain only letters, digits, hyphens, or spaces")
    ...
    public Address {
        ...
        if (street.length() > 255) {
            throw new IllegalArgumentException("Street must be at most 255 characters");
        }
        if (city.length() > 100) {
            throw new IllegalArgumentException("City must be at most 100 characters");
        }
        if (state.length() > 100) {
            throw new IllegalArgumentException("State must be at most 100 characters");
        }
        if (postalCode.length() > 20) {
            throw new IllegalArgumentException("Postal code must be at most 20 characters");
        }
        if (!postalCode.matches("^[A-Z0-9\\-\\s]+$")) {
            throw new IllegalArgumentException("Postal code must contain only letters, digits, hyphens, or spaces");
        }
    }
}